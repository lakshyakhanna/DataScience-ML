...
"""
"""No subjject Id and ICD codes"""
"""Log of ge has been Taken"""
"""EDA has been done"""
...
processed_final_dataset['DICHARGE_HOUR_BAND'] = processed_final_dataset['DICHARGE_HOUR_OF_DAY'].apply(hour_bins)
...
# One-Hot Encoding
final_dummy_data = pd.get_dummies(processed_final_dataset1, columns = ['ADMISSION_TYPE','ADMISSION_LOCATION','DISCHARGE_LOCATION','INSURANCE','RELIGION','MARITAL_STATUS','ETHNICITY','GENDER','LAST_CAREUNIT','DICHARGE_HOUR_BAND'])
...
X = final_dummy_data.drop(['READMISSION_60dAYS', 'READMISSION_30dAYS' , 'DAYS_TO_READMISSION','AGE'],axis = 1)
...
param_grid_test = {'criterion': ['gini'], 'max_features': [44], 'n_estimators': [250]}
rf_grid_estimator = model_selection.GridSearchCV(rf_estimator, param_grid_test, cv =10 , verbose = 1, n_jobs = 10,scoring = "roc_auc" )
...
sgd_param_grid = dict(loss = ['log','hinge','modified_huber'], penalty = ['L1','L2','elasticnet'] , alpha =  [10 ** a for a in range(-6, 7)])
...
log_grid_estimator.fit(X_train_scl,y_train)
print(log_grid_estimator.best_score_)
print(log_grid_estimator.score(X_train_scl, y_train))
...
y_pred_lasso = lasso_grid_estimator.predict(X_test_pca)
y_pred_lasso_test = pd.Series(y_pred_lasso).apply(lambda x : 0 if x < 0.5 else 1)

y_pred_log = log_grid_estimator.predict(X_test_scl)
...
#Lasso
metrics.roc_auc_score(y_test, y_pred_lasso_test)
